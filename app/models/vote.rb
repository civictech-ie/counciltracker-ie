class Vote < ApplicationRecord
  ...
  def seat
    councillor.seats.active_on(occurred_on).take
  end
  ...
  def implies_presence?
    %w[for against abstain not_voted].include? status.to_s
  end
  ...
  def update_attendance
    # TODO if somebody's voting, they're probably in attendance...
  end

  def redetermine_vote_result
    return true unless countable?
    voteable.redetermine_vote_result!
  end
end
