class Attendance < ApplicationRecord
  ...
  scope :possible, -> {
                     joins("LEFT JOIN meetings ON attendances.attendable_type = 'Meeting' AND attendances.attendable_id = meetings.id")
  ...
  def presentish?
    %w[present expected].include?(status)
  end

  private

  def councillor_on_council
    return true unless attendable
    unless councillor.active_on?(occurred_on)
